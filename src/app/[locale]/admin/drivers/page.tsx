...
export default function DriversPage() {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/drivers`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingDriver 
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/drivers/${editingDriver.id}` 
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/drivers/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleFileUpload = async (driverId: number, docType: string, file: File) => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/drivers/${driverId}/document`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      if (res.ok) {
        ...
        const updatedDrivers = await (await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/drivers`, {
           headers: { 'Authorization': `Bearer ${token}` }
        })).json();
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center font-bold text-lg overflow-hidden">
                          {driver.photo_path ? (
                            <img src={`${process.env.NEXT_PUBLIC_API_URL}${driver.photo_path}`} alt="Driver" className="w-full h-full object-cover" />
                          ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center text-center bg-slate-50">
                      ...
                      {editingDriver.photo_path && (
                        <div className="w-16 h-16 rounded-full overflow-hidden mb-2">
                          <img src={`${process.env.NEXT_PUBLIC_API_URL}${editingDriver.photo_path}`} alt="Profil" className="w-full h-full object-cover" />
                        </div>
                      )}
                    ...
                    <div className="border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center text-center bg-slate-50">
                      ...
                      {editingDriver.permit_document_path && (
                        <a href={`${process.env.NEXT_PUBLIC_API_URL}${editingDriver.permit_document_path}`} target="_blank" rel="noreferrer" className="text-xs text-blue-500 mb-2 hover:underline">Belgeyi Gör</a>
                      )}
                      <label className="cursor-pointer text-xs text-indigo-600 hover:text-indigo-800 font-medium">
                    ...
                    <div className="border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center text-center bg-slate-50">
                      ...
                      {editingDriver.identity_document_path && (
                        <a href={`${process.env.NEXT_PUBLIC_API_URL}${editingDriver.identity_document_path}`} target="_blank" rel="noreferrer" className="text-xs text-blue-500 mb-2 hover:underline">Belgeyi Gör</a>
                      )}
                      <label className="cursor-pointer text-xs text-indigo-600 hover:text-indigo-800 font-medium">
